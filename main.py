    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                paused = not paused
                fade(screen, SCREEN_WIDTH, SCREEN_HEIGHT, fade_in=False, text="Pausado", font=small_font) # Efeito de fade ao pausar

                if paused: # Só mostra "Pausado" se o jogo estiver realmente pausado
                    draw_text_center("PAUSADO", main_font, (255, 255, 0), screen, SCREEN_HEIGHT // 2)
                    pygame.display.flip() # Atualiza a tela para mostrar "PAUSADO"
                    # Espera um pouco ou até o usuário pressionar P novamente
                    temp_paused = True
                    while temp_paused:
                        for sub_event in pygame.event.get():
                            ...
                            if sub_event.type == pygame.KEYDOWN and sub_event.key == pygame.K_p:
                                paused = not paused # Despausa o jogo
                                temp_paused = False
                        clock.tick(30) # Limita o FPS enquanto pausado
                fade(screen, SCREEN_WIDTH, SCREEN_HEIGHT, fade_in=True, text="Continuando...", font=small_font) # Efeito de fade ao resumir
        ...
        if not paused:
            ...
            if ball.rect.x <= 0:
                ...
                ball.rect.x = SCREEN_WIDTH // 2
                ball.rect.y = SCREEN_HEIGHT // 2
                ball.velocity[0] = -ball.velocity[0] # Inverte a direção horizontal da bola
                # 'glow_timer' é usado para um efeito visual na bola após o ponto
                if hasattr(ball, 'glow_timer'): # Verifica se ball tem o atributo glow_timer
                    ball.glow_timer = 10
